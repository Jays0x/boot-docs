...
function CommandCenter() {
  ...
  return (
    <>
      {/* Search bar trigger */}
      <div
        className="flex justify-between items-center p-2 bg-[#181818] rounded-[10px] w-full max-w-[500px] lg:max-w-[500px] h-[35px] border border-white/10 cursor-pointer"
        onClick={() => setOpen(true)}
      >
        <div className="flex gap-3 items-center text-[#ffffff50]">
          <SearchIcon size={16} />
          <span className="text-sm text-[#ffffff30] w-full lg:w-[200px] max-w-[200px]">Search...</span>
        </div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-start sm:items-center z-50 p-4 overflow-auto"
          ...
          >
            <motion.div
              ...
              className="bg-[#181818] border border-white/10 rounded-xl p-4 w-full max-w-[500px] lg:max-w-[500px]"
            >
              {/* Header / Search input */}
              <div className="flex items-center gap-3 border-b border-white/10 pb-2 mb-3">
                ...
                <input
                  ...
                  className="bg-transparent outline-none text-white w-full placeholder:text-white/30"
                />
                <X
                  size={16}
